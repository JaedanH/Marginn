...
export function buildCompletePipelineReport(args: {
  ...
  const ebayScrapeFailed = stages.ebay_scrape?.status === "failed";
  const pricingFailed = stages.pricing?.status === "failed";
  if (warnings.length > 0 && (ebayScrapeFailed || pricingFailed)) {
    return buildPartialReport({
