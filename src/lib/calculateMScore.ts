/**
 * Client mirror of Edge `_shared/calculateMScore.ts` (M-Score recalc after comparable removal).
 */
...
const COAT_JACKET_MONTH: Record<number, number> = {
  1: 0.7, 2: 0.6, 3: 0.5, 4: 0.4, 5: 0.3, 6: 0.3, 7: 0.4, 8: 0.6, 9: 0.9, 10: 1.0, 11: 1.0, 12: 0.9,
};

const HOODIE_MONTH: Record<number, number> = {
  1: 0.9, 2: 0.8, 3: 0.7, 4: 0.6, 5: 0.5, 6: 0.5, 7: 0.6, 8: 0.8, 9: 1.0, 10: 1.0, 11: 0.9, 12: 0.9,
};

const TEE_MONTH: Record<number, number> = {
  1: 0.6, 2: 0.6, 3: 0.7, 4: 0.8, 5: 1.0, 6: 1.0, 7: 1.0, 8: 0.9, 9: 0.7, 10: 0.6, 11: 0.5, 12: 0.5,
};
...
function velocityScore(soldLast7: number, soldLast30: number): { score: number; trend: number } {
  ...
  const score = Math.min(10, (soldLast30 / 20) * 10 * 0.7 + Math.min(trend, 2) * 0.3);
