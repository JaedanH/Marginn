...
export default function ScanDetailModal({ scan, onClose, onMarkBought, onMarkSold }: ScanDetailModalProps) {
  ...
  const bought = Boolean(scan.bought_at);
  const sold = Boolean(scan.sold_at);
  const cost =
    scan.bought_price_gbp != null
      ? Number(scan.bought_price_gbp)
      : scan.buy_price_gbp != null
        ? Number(scan.buy_price_gbp)
        : null;
  const realized =
    sold && scan.sold_price_gbp != null && cost != null
      ? Number(scan.sold_price_gbp) - cost
      : null;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                {scan.roi != null && !Number.isNaN(Number(scan.roi)) ? (
                  <div>
                    <span className="text-sm font-medium text-gray-500">ROI (at scan):</span>
                    <p className="text-lg font-semibold text-black">{Number(scan.roi)}%</p>
                  </div>
                ) : null}
                {(bought || sold) && (
                  <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 space-y-1.5 text-sm">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Outcomes</p>
                    {bought && (
                      <p className="text-gray-800">
                        Bought
                        {scan.bought_at
                          ? ` · ${new Date(scan.bought_at).toLocaleDateString('en-GB')}`
                          : ''}
                    ...
                    {sold && (
                      <p className="text-gray-800">
                        Sold
                        {scan.sold_at ? ` · ${new Date(scan.sold_at).toLocaleDateString('en-GB')}` : ''}
                        {scan.sold_price_gbp != null ? ` · £${Number(scan.sold_price_gbp)}` : ''}
                      </p>
                    )}
                    {realized != null && !Number.isNaN(realized) ? (
                      <p className={`font-bold ${realized >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                        Realized profit: {realized >= 0 ? '' : '−'}£{Math.abs(Math.round(realized * 100) / 100)}
                      </p>
                    ) : null}
                  </div>
                )}
                {(onMarkBought || onMarkSold) && (
                  <div className="flex flex-wrap gap-2 pt-2">
                    {onMarkBought ? (
                      ...
                      >
                        {bought ? 'Marked bought' : 'Mark bought'}
                      </Button>
                    ) : null}
                    {onMarkSold ? (
                      <Button
                        variant="outline"
                        size="sm"
                        disabled={!bought || sold}
                        onClick={onMarkSold}
                        className="whitespace-nowrap"
                        title={!bought ? 'Mark as bought first' : undefined}
                      >
                        {sold ? 'Marked sold' : 'Mark sold'}
                      </Button>
                    ) : null}
                  </div>
                )}
                ...
                <tbody>
                  {marketplaces.map((marketplace, index: number) => (
                    <tr key={index} className="border-b border-gray-100">
                      ...
                      <td className="py-3 text-right text-gray-600">£{marketplace.lowPrice}</td>
                      <td className="py-3 text-right text-gray-600">£{marketplace.highPrice}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
