...
import { computeProfitBreakdown } from '../../lib/scanProfit';
import { pickBestBrandByAvgRoi, type BrandRoiRow, type BestBrandStat } from '../../lib/scanBrandAggregates';
import { runWatchlistResaleAlerts } from '../../lib/scanWatchlistAlerts';

/** kg CO₂ per scan (dashboard metric) */
const CO2_KG_PER_SCAN = 2.1;

interface ScanRecord extends ScanRowWithOutcomes {
  id: string;
  created_at: string;
  brand_name: string;
...
export default function DashboardPage() {
  ...
  const [bestBrandRoi, setBestBrandRoi] = useState<BestBrandStat | null>(null);
    ...
    const fetchDashboardData = async () => {
      ...
      const [
        ...
        supabase
          .from('scans')
          .select(
            'profit_gbp, bought_at, sold_at, sold_price_gbp, bought_price_gbp, buy_price_gbp, expected_resale_gbp',
          )
          .eq('user_id', userId)
          .limit(5000),
        supabase
        ...
        supabase.from('scans').select('id, brand_name, roi').eq('user_id', userId).not('roi', 'is', null).limit(5000),
      ]);
      ...
      const profitRows = (profitsRes.data as ScanRowWithOutcomes[]) ?? [];
      const { totalTracked } = computeProfitBreakdown(profitRows);
      const co2Saved = Math.round(scanCount * CO2_KG_PER_SCAN * 10) / 10;

      setStats({ scansRemaining, totalProfit: Math.round(totalTracked * 100) / 100, co2Saved });
      ...
      setBestBrandRoi(pickBestBrandByAvgRoi((roiBrandRes.data as BrandRoiRow[]) ?? []));

      await runWatchlistResaleAlerts({ supabase, userId, showToast });
    };
  ...
  }, [user, session?.user?.id, profile, lastMonthStartIso, showToast]);
  ...
  useEffect(() => {
    ...
    const onSaved = () => void refetchRecent();
    document.addEventListener('visibilitychange', onVis);
    window.addEventListener('marginn:scan-saved', onSaved);
    return () => {
      cancelled = true;
      document.removeEventListener('visibilitychange', onVis);
      window.removeEventListener('marginn:scan-saved', onSaved);
    };
  ...
  return (
    <DashboardLayout>
      <div className="space-y-10">
        ...
        <BrandPerformanceCard best={bestBrandRoi} />
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-2 leading-snug max-w-[14rem]">
                  Includes modelled scans, bought-not-sold pipeline (scan profit), and realized sales
                  (sold price minus buy).
                </p>
              </div>
